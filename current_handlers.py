...
WSPORT = "1555"
WS_URL = "http://192.168.1.133"
WSONLINE_URL = "ws://192.168.1.133:8000"
...
handler_layout_lang = {
                        ...
                        "Elements": [
                            ...
                            ,
                            {
                                ...
                                "width": "match_parent",
                                "weight": "0",
                                "Value": "Handler type|@handler_types",
                                "Variable": "type",
                                "gravity_horizontal": "left"
                            }
                            ,
...
def configuration_open(hashMap,_files=None,_data=None):    
    ...
    hashMap.put("url_configuration",'<html><body>Configuration URL: <a href="'+link+'">' +link+ '</a></body></html>')  
    
    if "ConfigurationFileName" in session["configuration"]['ClientConfiguration']:
        hashMap.put("download_configuration",'The configuration file can be downloaded here: <a href="/download_file?filename='+Path(filename).name+'" target="_blank" download="'+ session["configuration"]['ClientConfiguration']['ConfigurationFileName']+ '">download configuration</a>')     
    elif session["filename_base"]!=None:
        hashMap.put("download_configuration",'The configuration file can be downloaded here: <a href="/download_file?filename='+Path(filename).name+'" target="_blank" download="'+ session["filename_base"]+ '">download configuration</a>')     
    else:    
        hashMap.put("download_configuration",'The configuration file can be downloaded here: <a href="/download_file?filename='+Path(filename).name+'" target="_blank" ">download configuration</a>')     
        ...
        if "GitHubHandlers" in new_configuration["ClientConfiguration"]:
            ...
            if len(handlers_url)>0:
                ...
                if handlers_txt!=None:
                    ...
                    hashMap.put("toast", "Error fetching data from GitHub")
        ...
        if hashMap.containsKey("_cookies"):
            ...
            if str(jcookie.get("ui_to_github")).lower()=="true":
                if new_configuration["ClientConfiguration"].get("ConfigurationFileName")!="" and new_configuration["ClientConfiguration"].get("ConfigurationFileName")!=None:
                    ...
                    if r==False:
                        hashMap.put("toast","Failed to send to GitHub")
...
def configuration_input(hashMap,_files=None,_data=None):
    ...
    #     hashMap.put("toast","var1="+session["var1"]+", var2="+var2)
        ...
        session["configuration"]={"ClientConfiguration":
        {"ConfigurationName": 'New configuration',"ConfigurationDescription": "Creating a new configuration", "ConfigurationVersion": "0.0.1", "Processes":[
                            {
                        "type": "Process",
...
def make_timers_table(_table):
    ...
    "columns": [
        ...
        {
        "name": "PyTimerTaskDef",
        "header": "Alias(in common handlers)",
        "weight": "1",
        "gravity":"left"
    },
        {
...
def process_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_save" or hashMap.get("listener")=="btn_close":
        ...
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"main_screen", "key":"Processes", "reopen":True},ensure_ascii=False))
        ...
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"screen_form","key":uid,"reopen":True},ensure_ascii=False))  
        session["current_process_name"] = "New screen"
        ...
        hashMap.put("SetTitle","New screen - *")
        ...
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"screen_form","key":row['uid'],"reopen":True,"no_close":True},ensure_ascii=False))  
        ...
        try:
            ...
            hashMap.put("toast","clipboard error")    
...
def screen_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_save" or hashMap.get("listener")=="btn_close":
        ...
        hashMap.put("CloseTab",closeuid)
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"process_form", "key":session["current_parent"][1][0]['uid'], "reopen":True},ensure_ascii=False))
        ...
        if session["opened_element_uid"] != None:
                hashMap.put("toast","Only 1 item can be opened.")
                return hashMap
            
        session["opened_element_uid"] = uid

        if session["layouts_edit"]:
            hashMap.put("OpenScreen",json.dumps({"process":"Layouts(Containers)","screen":"element_form","key":uid,"reopen":True,"no_close":True},ensure_ascii=False))  
        else:    
            hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"element_form","key":uid,"reopen":True,"no_close":True},ensure_ascii=False))  
        ...
        hashMap.put("SetTitle","New screen element - *")
        ...
        if hashMap.containsKey("selected_line_id"):
            ...
            if session["layouts_edit"]:
                hashMap.put("OpenScreen",json.dumps({"process":"Layouts(Containers)","screen":"element_form","key":row['uid'],"reopen":True,"no_close":True},ensure_ascii=False))  
            else:    
                hashMap.put("OpenScreen",json.dumps({"Processes":"Processes","screen":"element_form","key":row['uid'],"reopen":True,"no_close":True},ensure_ascii=False))  
            #current_screen_name = row.get("Name")
            #hashMap.put("screen_name",current_screen_name)
        ...
        hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Handler"},ensure_ascii=False))
        ...
        hashMap.put("RefreshScreen","")
        hashMap.put("callSelectTab","Handlers")
        hashMap.put("SelectTab","Handlers")
...
def element_input(hashMap,_files=None,_data=None):
        ...
        if session.get("layouts_edit")==True and get_key(element_base,hashMap.get('type')) == 'LinearLayout':
            if session["current_parent"]==None:
                ...
                if row[session["elements_table_id"]]['Variable']=='':
                    hashMap.put("toast","You must specify a container variable. The variable is generated automatically.")
    ...
    if hashMap.get("listener")=="btn_save" :
        ...
        if session["current_parent"][1]==None:
            hashMap.put("layouts_table",json.dumps(make_onefield_table(session["configuration"]["ClientConfiguration"]['Layouts'],"Variable","Variable"),ensure_ascii=False))
            hashMap.put("OpenScreen",json.dumps({"process":"Layouts(Containers)","screen":"main_form", "key":"Layouts(Containers)", "reopen":True},ensure_ascii=False))
        else:    
            if session["current_parent"][1][0].get("type") == "Operation":
                ...
                hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"screen_form", "key":session["current_parent"][1][0]['uid'], "reopen":True},ensure_ascii=False))
            else:    
                ...
                hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"element_form", "key":session["current_parent"][1][0]['uid'], "reopen":True},ensure_ascii=False))
    ...
    if  hashMap.get("listener")=="btn_close":
        ...
        if session["current_parent"]==None:
            ...
            if session["current_element"].get("type") == "Operation":
                ...
                hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"screen_form", "key":session["current_parent"][0]['uid'], "reopen":True},ensure_ascii=False))
            else:    
                ...
                hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"element_form", "key":session["current_parent"][0]['uid'], "reopen":True},ensure_ascii=False))
        ...
        hashMap.put("SetTitle","New layout element- *")
        ...
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"element_form","key":row['uid'],"reopen":True,"no_close":True},ensure_ascii=False))  
        hashMap.put("SetTitle",row.get("type"))
            ...
            if session["current_parent"][1][0].get("type") == "Operation":
                ...
                if get_key(element_base,hashMap.get('type')) == 'ModernEditText':
                    if hashMap.get("Value")=="" or hashMap.get("Value")==None:
                        template = json.dumps({"hint":"Field name", "default_text":"default_value"},ensure_ascii=False)
...
def process_open(hashMap,_files=None,_data=None):
    ...
    if not session["current_element"] == None:
        hashMap.put("screens_table",json.dumps(make_onefield_table(session["current_element"]["Operations"],"Name","Screen"),ensure_ascii=False))
    else:
        hashMap.put("screens_table",json.dumps(make_onefield_table([],"Name","Screen"),ensure_ascii=False))
...
def processes_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_add_process":
        ...
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"process_form", "key":uid, "reopen":True},ensure_ascii=False))  
        session["current_process_name"] = "New process"
        ...
        if hashMap.containsKey("selected_line_id"):
            ...
            if row.get("type") == "Process":   
                ...
                hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"process_form", "key":session["processes_table"][session["processes_table_id"]].get("uid"), "reopen":True},ensure_ascii=False))  
...
def mediafiles_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_add_mediafile":
        ...
        hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Adding file"},ensure_ascii=False))
        hashMap.put("ShowDialog","")
        hashMap.put("key","")
...
def activecv_process_open(hashMap,_files=None,_data=None):
    ...
    if not session["current_element"] == None:
        hashMap.put("steps_table",json.dumps(make_onefield_table(session["current_element"]["CVFrames"],"Name","ActiveCV step"),ensure_ascii=False))
    else:
        hashMap.put("steps_table",json.dumps(make_onefield_table([],"Name","ActiveCV Step"),ensure_ascii=False))
...
def activecv_process_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_save" or hashMap.get("listener")=="btn_close":
        ...
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"step_form","key":uid,"reopen":True},ensure_ascii=False))  
        hashMap.put("step_name","New step")
        ...
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"step_form","key":row['uid'],"reopen":True,"no_close":True},ensure_ascii=False))  
        current_screen_name = row.get("Name")
...
def step_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_save" or hashMap.get("listener")=="btn_close":
        ...
        hashMap.put("CloseTab",closeuid)
        hashMap.put("OpenScreen",json.dumps({"process":"Processes","screen":"activecv_process_form", "key":session["current_parent"][1][0]['uid'], "reopen":True},ensure_ascii=False))
...
style_layout =  {
            ...
            "Elements": [
                ...
                },
                {
                    ...
                    "width": "wrap_content",
                    "weight": "0",
                    "Value": "Horizontal alignment|@gravity_horizontal_elements",
                ...
                },
                {
                    ...
                    "width": "wrap_content",
                    "weight": "0",
                    "Value": "Number of characters|@NumberPrecision",
...
def styles_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_add_style":
        ...
        hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Adding a Style Template"},ensure_ascii=False))
        hashMap.put("ShowDialog","")
        ...
        if hashMap.containsKey("selected_line_id"):
            ...
            hashMap.put("ShowDialogLayout",json.dumps(style_layout,ensure_ascii=False))
            hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Editing a timer"},ensure_ascii=False))
            hashMap.put("ShowDialog","")
...
def styles_open(hashMap,_files=None,_data=None):
    ...
    if "StyleTemplates" in session["configuration"]['ClientConfiguration']:
        hashMap.put("styles_table",json.dumps(make_onefield_table(session["configuration"]['ClientConfiguration']["StyleTemplates"],"name","Name"),ensure_ascii=False))
    else:    
        hashMap.put("styles_table",json.dumps(make_onefield_table([],"name","Name"),ensure_ascii=False))
            ...
            "Elements": [

                {
                ...
                },
                 {
                    ...
                    "width": "wrap_content",
                    "weight": "0",
                    "Value": "Period|@PyTimerTaskPeriod",
...
menu_layout =  {
            ...
            "Elements": [

                {
                    ...
                    "width": "match_parent",
                    "weight": "0",
                    "Value": "Element|@main_menu_elements",
                    "Variable": "MenuItem",
                    "gravity_horizontal": "left"
                },
                 {
...
def timers_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_add_timer":
        ...
        hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Adding timer"},ensure_ascii=False))
        hashMap.put("ShowDialog","")
...
def modules_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_add_file":
        ...
        hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Adding a module"},ensure_ascii=False))
        hashMap.put("ShowDialog","")
        hashMap.put("key","")
        ...
        if hashMap.get("dialog_mode")  == "add_files":
            ...
            hashMap.put("RefreshScreen","")
                ...
                if 'base64' in dialog_values:
                    ...
                    hashMap.put("toast", "Handlers have been updated")
...
<body>
<h3 style="font-size:14px; ">The easiest way to work with python handlers is pythonscript, it does not require binding to additional files. But if you still need to develop in an external IDE, you can bind external files to the Simple project via GitHub</h3>
<h3 style="font-size:14px; "><u>МYou can use GitHub private or public.</u></h3>
<ol>
  <li style="font-size:12px; ">Specify the URL of the main handler file on GitHub in this format:
<b>https://api.github.com/repos/your_github/your_repo/contents/file_name.py</b>
</li>
  <li style="font-size:12px; ">If necessary (if not public GitHub) generate and specify a token (Settings - Developer settings - Personal access tokens – Tokens (classic) – Generate new token)</li>
  <li style="font-size:12px; ">If additional modules are required, please specify the key and URL of the additional module file.</li>
  <li style="font-size:12px; ">After committing changes to GitHub, when saving the configuration, the modified module texts will be applied in the configuration</li>
</ol>  
</body>
</html>"""
...
def layouts_open(hashMap,_files=None,_data=None):
    ...
    if "Layouts" in session["configuration"]["ClientConfiguration"]:
        ...
        hashMap.put("layouts_table",json.dumps(make_onefield_table([],"Variable","Variable"),ensure_ascii=False))
...
<p>This constructor is designed to create configurations for SimpleUI (Android devices) and SimpleWEB (desktop) and is a configuration on SimpleWEB.</p>
<div style="background-color: Azure;  padding-top: 10px;  padding-bottom: 10px;">

<p><font style="font-size: 20px !important;">GitHub of this constructor <a href="https://github.com/dvdocumentation/web_simple_editor" target="_blank">https://github.com/dvdocumentation/web_simple_editor </a></font></p>
...
<p><font style="font-size: 20px !important;">Telegram channel(RU) <a href="https://t.me/devsimpleui" target="_blank">https://t.me/devsimpleui</a></font></p>



</div>

<p>(c) Dmitry Votontsov, 2025 </p>
...
def source_code_save(hashMap,_files=None,_data=None):
    ...
    try:
        ...
        hashMap.put("toast", "Configuration updated")  
    except:
        hashMap.put("toast", "Error updating configuration!")  
...
def menu_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener")=="btn_add_menu":
        ...
        hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Adding a menu"},ensure_ascii=False))
        hashMap.put("ShowDialog","")
        ...
        if hashMap.containsKey("selected_line_id"):
            ...
            hashMap.put("ShowDialogLayout",json.dumps(menu_layout,ensure_ascii=False))
            hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Editing the menu"},ensure_ascii=False))
            hashMap.put("ShowDialog","")
...
def user_open(hashMap,_files=None,_data=None):
    ...
    if hashMap.containsKey("_cookies"):
        ...
        hashMap.put("GetCookies","")
        hashMap.put("toast","Reopen the tab")
...
def debugws_connect(hashMap,_files=None,_data=None):
    if not hashMap.containsKey("token"):
        hashMap.put("ErrorMessage","Token not set")
        return hashMap
    session["token"] = hashMap.get("token")
...
def debugws_preview(hashMap,_files=None,_data=None):
    if hashMap.containsKey("_debug_preview"):
        if hashMap.get("_debug_preview")=="true":
            hashMap.put("toast","Visualization is disabled")
            hashMap.put("_debug_preview","false")
        else:
            hashMap.put("toast","Visualization enabled")
...
def search_variables(hm,layout):
    ...
    "cardsdata":[{"txt1":"Card 1","txt2":"Contents of card 1"},{"txt1":"Card 2","txt2":"Card Contents 2"}]
            ...
            "tabledata":[{"txt1":"Card 1","txt2":"Contents of card 1"},{"txt1":"Card 2","txt2":"Card Contents 2"}]
                        ...
                        "Elements": [
                            {
                                ...
                                "width": "match_parent",
                                "weight": "0",
                                "Value": "Number of columns|@columns",
                                "Variable": "columns",
                                "gravity_horizontal": "left"
                            },
                            {
        ...
        if "draw_mode" in session:     
            if session["draw_mode"] == "address":
                ...
                if session["current_row"]!=None:
                    ...
                    hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Editing a cell"},ensure_ascii=False))
                    hashMap.put("ShowDialog","") 
...
def cell_input(hashMap,_files=None,_data=None):
    ...
    if hashMap.get("listener") == "btn_row":
        ...
        hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Adding a row of cells"},ensure_ascii=False))
        hashMap.put("ShowDialog","") 
    ...
    if hashMap.get("listener") == "btn_text":
        ...
        hashMap.put("ShowDialogStyle",json.dumps({"yes":"Save","no":"Cancel","title":"Text input"},ensure_ascii=False))
        hashMap.put("ShowDialog","") 
    ...
    if hashMap.get("listener") == "onResultPositive": 
        ...
        if file_w == None or file_h==None:
            hashMap.put("toast","Image dimensions not specified")
            return hashMap
        ...
        with open(session["sugfilename"] , 'w',encoding="utf-8") as f:
            ...
            hashMap.put("download_sug",'sug-file here: <a href="/download_file?filename='+Path(session["sugfilename"]).name+'" target="_blank" download="vector_drawable.sug">download sug-file</a>')     
